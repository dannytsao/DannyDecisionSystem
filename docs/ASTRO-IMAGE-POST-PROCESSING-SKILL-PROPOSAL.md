...
- Companion fallback: 若 DNGLab 不可用或 DNG 驗證失敗，exporter 會改用 Siril `savetif` 產生 Lightroom 較相容的 16-bit 整數 TIFF；只有 DNG 與 TIFF 都失敗時才停止並寫入 `Failed/failed-report.md`。
- M31 grouped execution: 將 1,150 張混合資料按日期、濾鏡與 RA/DEC tile 拆成 41 個隔離 run 後，註冊成功率由 35.6% 提升至 92.7%（1,066/1,150）；這是分組有效性的實際證據，不代表每個 tile 都已完成獨立 QA。
